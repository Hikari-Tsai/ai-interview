...
import {renderAnswerText} from '../../src/lib/prose.ts';
...
export function validateAnswer(value:unknown,q:Question,groundedUrls:string[],allowHistoricalRepo=false):Answer {
 ...
 for(const [locale,text] of Object.entries(answer.locales)) {
  ...
  for(const section of ['principle','tradeoff','implementation','production'] as const){
   if(text[section].includes('$')){
    try{renderAnswerText(text[section]);}
    catch(error){throw new Error(`${q.id}/${locale}/${section}: ${error instanceof Error?error.message:'Invalid answer formula'}`);}
   }
  }
 }
