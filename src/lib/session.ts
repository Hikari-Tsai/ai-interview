...
export function readFilters(params: URLSearchParams): Filters {
 ...
 return { tags:[...new Set(params.getAll('tag').filter(Boolean))], match:params.get('match')==='all'?'all':'any', company:companyName(params.get('company')||''), search:params.get('search')||'', ready:params.get('ready')==='1', mode:params.get('mode')==='random'?'random':'sequential', seed:Number.isSafeInteger(seed)?seed:1, anchor:params.get('anchor')||'' };
}
export function filterCards(cards: Card[], filters: Filters,locale?:Locale): Card[] {
 const query=filters.search.trim().toLocaleLowerCase();
 return cards.filter(card=>card.active && (!filters.company||card.companies.some(name=>companyName(name)===companyName(filters.company))) && (!filters.ready||(card.answer?.status==='ready'&&(!locale||!!card.answer.locales[locale]))) &&
